...
async def run_poc_request(
    ...
    async def _run_one(nonce: int) -> dict[str, Any]:
        ...
        return await engine_client.poc_compute(
            ...
            k_dim=k_dim,
            timeout=None,
            # PoC must yield to chat under priority scheduling.
            priority=POC_REQUEST_PRIORITY,
        )

    # IMPORTANT: do not pass a frontend timeout into engine_client.poc_compute.
    # We implement a *local* wait timeout using asyncio.shield so a timeout does
    # NOT cancel the underlying scheduler requests.
    ...
    return build_artifacts_obj(all_nonces, all_vectors_b64)
...
async def _generation_loop(
    ...
    try:
        while not stop_event.is_set():
            nonces = pending_nonces if pending_nonces is not None else nonce_iter.take(1)
...
async def generate(
    ...
    try:
        computed_artifacts = await _compute_artifacts_chunk(
            engine_client,
            body.nonces,
            body.block_hash,
            body.block_height,

...
def _pack_3bit_vectorized(
    ...
    if head_size < padded:
        indices = torch.nn.functional.pad(
            indices, (0, padded - head_size), value=0)
...
class TritonAttentionImpl(AttentionImpl):
    ...
    def _decode_turboquant_cache(
        ...
        from vllm.v1.attention.ops.triton_turboquant import turboquant_decode
        ...
        # Gather all blocks referenced by block_table (may have duplicates).
        # This avoids unique() which is incompatible with CUDA graph capture.
    ...
    def _encode_turboquant_cache(
        ...
        from vllm.v1.attention.ops.triton_turboquant import turboquant_encode
        ...
        for kv_idx, (tensor, state) in enumerate([
            ...
            # Triton encode normal channels
            indices, norms = turboquant_encode(
                normal_x.float(), state.PiT, state.codebook, state.boundaries)
            ...
            if bits == 4:
                ...
                packed = (flat_indices[:, 0::4]
                          | (flat_indices[:, 1::4] << 2)
                          | (flat_indices[:, 2::4] << 4)
                          | (flat_indices[:, 3::4] << 6))
            elif bits == 3:
                packed = _pack_3bit_vectorized(
                    flat_indices, normal_size, packed_bytes)
            else:
            ...
            if outlier_x is not None:
                outlier_bytes = outlier_x.reshape(
                    N, n_outliers).to(torch.bfloat16).view(
                    torch.uint8).reshape(N, n_outliers * 2)
                parts.append(outlier_bytes)
            parts.append(packed)
            norm_bytes_data = norms.reshape(N).to(torch.float16).view(

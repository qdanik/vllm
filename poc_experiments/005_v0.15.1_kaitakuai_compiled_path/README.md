...
## ⚠️ Data caveat — two PoC-input clusters

The 8 datasets in this folder were collected with **two different `block_hash` / `public_key` pairs**, which seed the synthetic embeddings inside `generate_inputs(...)`. Datasets with different seeds produce conceptually different artifacts and cannot be compared directly.

| cluster | block_hash | public_key | datasets |
|---|---|---|---|
| A | `TEST_BLOCK` | `test_pub_keys` | `a100_aot{0,1}.json`, `b200_aot{0,1}.json` |
| B | `artifact_collection_block_v1` | `artifact_collection_pk_v1` | `h100_aot{0,1}.json`, `rtx_aot{0,1}.json` |

Comparisons across the cluster boundary (A↔B) give mean L2 ≈ 1.4 with 98% above threshold — that's not GPU drift, it's two independent uniform-on-sphere distributions getting compared. **Only within-cluster pairs (12 of 28) are reported.**

If we need a full 8×8 matrix, re-run one cluster against the other's `(block_hash, pk)`.
...
- GPUs: RTX PRO 6000 Blackwell, H100 PCIe
...
1. **Eager mode is bit-stable cross-GPU within each cluster.** A100↔B200: 0.086 mean L2 (1/1000 over thr). H100↔RTX: 0.024 mean L2 (1/1000 over thr). Both far under threshold 0.4.
...
- Cluster A (TEST_BLOCK): `a100_aot1.json`, `a100_aot0_eager.json`, `b200_aot1.json`, `b200_aot0_eager.json`
- Cluster B (artifact_collection_block_v1): `h100_aot1.json`, `h100_aot0_eager.json`, `rtx_aot1.json`, `rtx_aot0_eager.json`

...
pytestmark = pytest.mark.skipif(
    not torch.cuda.is_available(), reason="GPU tests require CUDA"
)
...
def test_inputs_determinism():
    ...
    inputs1 = generate_inputs(
        BLOCK_HASH, PUBLIC_KEY, nonces, dim=128, seq_len=16, device=device
    )
    inputs2 = generate_inputs(
        BLOCK_HASH, PUBLIC_KEY, nonces, dim=128, seq_len=16, device=device
    )
...
def test_inputs_different_nonces():
    ...
    inputs1 = generate_inputs(
        BLOCK_HASH, PUBLIC_KEY, [1], dim=128, seq_len=16, device=device
    )
    inputs2 = generate_inputs(
        BLOCK_HASH, PUBLIC_KEY, [2], dim=128, seq_len=16, device=device
    )
...
def test_different_block_hash_produces_different_inputs():
    ...
    device = torch.device("cuda:0")
    inputs1 = generate_inputs(
        "hash1", PUBLIC_KEY, [0], dim=128, seq_len=16, device=device
    )
    inputs2 = generate_inputs(
        "hash2", PUBLIC_KEY, [0], dim=128, seq_len=16, device=device
    )
    assert not torch.allclose(inputs1, inputs2)
...
def test_different_public_key_produces_different_inputs():
    ...
    device = torch.device("cuda:0")
    inputs1 = generate_inputs(
        BLOCK_HASH, "node1", [0], dim=128, seq_len=16, device=device
    )
    inputs2 = generate_inputs(
        BLOCK_HASH, "node2", [0], dim=128, seq_len=16, device=device
    )
    assert not torch.allclose(inputs1, inputs2)
...
def test_cpu_gpu_inputs_match():
    ...
    inputs_cpu = generate_inputs(
        BLOCK_HASH, PUBLIC_KEY, [0, 1, 2], dim=128, seq_len=16, device=cpu
    )
    inputs_gpu = generate_inputs(
        BLOCK_HASH, PUBLIC_KEY, [0, 1, 2], dim=128, seq_len=16, device=gpu
    )
...
def test_random_pick_indices_determinism():
    ...
    idx1 = random_pick_indices(
        BLOCK_HASH, PUBLIC_KEY, nonces, dim=1000, k=64, device=device
    )
    idx2 = random_pick_indices(
        BLOCK_HASH, PUBLIC_KEY, nonces, dim=1000, k=64, device=device
    )
...
def test_random_pick_indices_shape():
    ...
    indices = random_pick_indices(
        BLOCK_HASH, PUBLIC_KEY, nonces, dim=1000, k=k, device=device
    )
...
def test_random_pick_indices_range():
    ...
    indices = random_pick_indices(
        BLOCK_HASH, PUBLIC_KEY, [0, 1, 2], dim=dim, k=k, device=device
    )
...
def test_random_pick_indices_uniqueness():
    ...
    indices = random_pick_indices(
        BLOCK_HASH, PUBLIC_KEY, [0], dim=1000, k=k, device=device
    )
...
def test_random_pick_indices_different_nonces():
    ...
    idx1 = random_pick_indices(
        BLOCK_HASH, PUBLIC_KEY, [0], dim=1000, k=64, device=device
    )
    idx2 = random_pick_indices(
        BLOCK_HASH, PUBLIC_KEY, [1], dim=1000, k=64, device=device
    )
...
def test_random_pick_indices_invalid_k():
    ...
    with pytest.raises(ValueError):
        random_pick_indices(
            BLOCK_HASH, PUBLIC_KEY, [0], dim=1000, k=1001, device=device
        )
...
def test_cpu_gpu_random_pick_indices_match():
    ...
    idx_cpu = random_pick_indices(
        BLOCK_HASH, PUBLIC_KEY, [0, 1, 2], dim=1000, k=64, device=cpu
    )
    idx_gpu = random_pick_indices(
        BLOCK_HASH, PUBLIC_KEY, [0, 1, 2], dim=1000, k=64, device=gpu
    )
...
def test_uniform_distribution_mean_variance():
    ...
    assert abs(mean - expected_mean) < 0.005, (
        f"Mean {mean} too far from {expected_mean}"
    )
...
def test_generate_inputs_gaussian_distribution():
    ...
    for batch_start in range(0, 1000, 100):
        ...
        inputs = generate_inputs(
            BLOCK_HASH, PUBLIC_KEY, nonces, dim=dim, seq_len=seq_len, device=device
        )
...
def test_generate_inputs_per_nonce_independence():
    ...
    inputs = generate_inputs(
        BLOCK_HASH, PUBLIC_KEY, [0, 1], dim=dim, seq_len=seq_len, device=device
    )
...
def test_random_pick_indices_uniform_selection():
    ...
    for batch_start in range(0, n_nonces, 1000):
        ...
        indices = random_pick_indices(
            BLOCK_HASH, PUBLIC_KEY, nonces, dim=dim, k=k, device=device
        )
    ...
    assert chi2 < critical_value, (
        f"Chi-square {chi2} exceeds critical value {critical_value}"
    )
...
def test_random_pick_indices_coverage():
    ...
    for batch_start in range(0, n_nonces, 500):
        ...
        indices = random_pick_indices(
            BLOCK_HASH, PUBLIC_KEY, batch_nonces, dim=dim, k=k, device=device
        )
        for idx in indices.flatten():
            counts[idx] += 1
    ...
    assert ratio < 3.0, (
        f"Selection ratio {ratio} too extreme (suggests non-uniform distribution)"
    )
...
def test_haar_rotation_output_uniform_on_sphere():
    ...
    expected_var = 1.0 / k
    mean_var = component_vars.mean().item()
    assert abs(mean_var - expected_var) < 0.02, (
        f"Mean variance {mean_var} too far from {expected_var}"
    )
